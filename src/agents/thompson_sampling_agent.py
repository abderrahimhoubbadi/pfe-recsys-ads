...
class ThompsonSamplingAgent(BaseMOOAgent):
    ...
    def __init__(
        ...
        # Per-arm inverse covariance (no need to store A with Sherman-Morrison)
    ...
    def predict_all(self, context: np.ndarray) -> List[Dict[str, Tuple[float, float]]]:
        ...
        for arm in range(self.n_arms):
            ...
            for obj in self.objectives:
                ...
                # This avoids an O(d^3) 384-dimensional multivariate sample!
                variance = (self.v**2) * (x @ A_inv_a @ x)
                sampled_value = np.random.normal(float(mean), np.sqrt(float(variance)))
