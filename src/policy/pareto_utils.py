...
def is_dominated(v1: np.ndarray, v2: np.ndarray, maximize: bool = True) -> bool:
    ...
    """
    # ULTRA-FAST PATH for 2-objective optimization
    # Bypassing Numpy overhead saves ~120ms per global model iteration
    if len(v1) == 2:
        if maximize:
            return (v2[0] >= v1[0] and v2[1] >= v1[1]) and (
                v2[0] > v1[0] or v2[1] > v1[1]
            )
        else:
            return (v2[0] <= v1[0] and v2[1] <= v1[1]) and (
                v2[0] < v1[0] or v2[1] < v1[1]
...
def is_pareto_optimal(
    solution: np.ndarray, all_solutions: List[np.ndarray], maximize: bool = True
) -> bool:
    """
...
def get_pareto_front(
    solutions: List[Tuple[int, np.ndarray]], maximize: bool = True
...
def update_pareto_set(
    current_set: List[Tuple[int, np.ndarray]],
...
def crowding_distance(front: List[Tuple[int, np.ndarray]]) -> Dict[int, float]:
    ...
    for m in range(n_objectives):
        ...
        for i in range(1, n_solutions - 1):
            ...
            distances[arm] += (
                sorted_front[i + 1][1][m] - sorted_front[i - 1][1][m]
            ) / obj_range
...
def chebyshev_distance(
    point: np.ndarray, reference: np.ndarray, weights: np.ndarray
) -> float:
    """
